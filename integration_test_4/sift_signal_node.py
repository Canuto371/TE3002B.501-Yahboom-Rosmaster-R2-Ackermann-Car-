...
class SIFTSignalNode(Node):
    ...
    def extract_main_rectangle_roi(self, frame):
        ...
        for contour in contours:
            ...
            if len(approx) < 4 or len(approx) > 12:
                continue
            '''lol'''
            area_ratio = bbox_area / float(frame_area) # quitar
            
            candidates.append(
                (area, x, y, w, h, area_ratio) # -> sin area_ratio
            )
        ...
        _, x, y, w, h, area_ratio = candidates[0] # -> sin area_ratio
        '''.'''
        ...
        '''lol'''
        return roi, area_ratio # -> sin area_ratio
        '''.'''
    ...
    def classify(self, frame):
        ...
        '''lol'''
        result = self.extract_main_rectangle_roi(frame) # roi = self.extract_main_rectangle_roi(frame)

        if result is None: # if roi is None
            return "unknown", 0, 0.0 # return "unknown", 0
            
        roi, area_ratio = result # quitar
        '''.'''
    ...
    # Convertir a escala de grises
        ...
        '''lol'''
        if des_test is None:
            return "unknown", 0, area_ratio # sin area_ratio
        '''.'''
        best_label = "unknown"
        best_score = 0
    ...
    # ======================================================
        ...
        if best_label in ["agv_area", "loading"]:
            ...
            if best_score < 20:
                best_label = "unknown"
        '''lol'''
        return best_label, best_score, area_ratio # sin area_ratio
        '''.'''
    # ==========================================================
    # PROCESS FRAME
    # ==========================================================
    ...
    def process_frame(self):
        ...
        if not ret:
            return
        '''lol'''
        label, score, area_ratio = self.classify(frame) # sin area_ratio
        '''.'''
        # Temporal filtering
        ...
        msg = String()
        '''lol'''
        msg.data = f"{label},{score},{area_ratio:.3f}" # msg.data = f"{label},{score}"
        '''.'''
        ...
        '''lol'''
        text = f"{label} ({score}) A={area_ratio:.3f}" # text = f"{label} ({score})"
        '''.'''
        cv2.putText(
            display,
            text,
